 ...
 * CN 节点检查存活状态
 */
public class CNHandlerStartegy implements ServiceRoleStrategy {
    
    private static final Logger logger = LoggerFactory.getLogger(CNHandlerStartegy.class);
    ...
    public void handlerServiceRoleCheck(ClusterServiceRoleInstanceEntity roleInstanceEntity,
        ...
        if (roleInstanceEntity.getHostname().equals(feMaster)
            ...
            try {
                List<ProcInfo> computeNodes = OlapUtils.showComputeNodes(feMaster);
                resolveProcInfoAlert(roleInstanceEntity.getServiceRoleName(), computeNodes, map);
            } catch (Exception e) {
    ...
    private void resolveProcInfoAlert(String serviceRoleName, List<ProcInfo> computeNodes,
                                      Map<String, ClusterServiceRoleInstanceEntity> map) {
        for (ProcInfo frontend : computeNodes) {
